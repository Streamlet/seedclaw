...
- agent: Spawn a sub-agent to complete a task. (preferred, must be your first choice)
  ...
  Available sub-agents will be listed in `Available Sub-Agents` section. If no such section, it indicates no sub-agents available.
  You are FORBIDDEN to invent, guess, or assume any agent name.

- shell: Execute a shell command.
  Parameters: command (string), arguments (array of strings)
  Example: {"command": "ls", "arguments": ["-a", "-l"]}
  Available commands are: {{COMMANDS}}
  Never try other agents. You are FORBIDDEN to invent, guess, or assume any command.
